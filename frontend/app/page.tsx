...
export default function LandingPage() {
  ...
  const bg = dark ? '#111' : 'linear-gradient(135deg, #FDF4FF 0%, #EEF2FF 40%, #E0F2FE 100%)';
  const text = dark ? '#f0f0f0' : '#0A2540';
  const sub = dark ? '#aaa' : '#555';
  const border = dark ? '#2a2a2a' : '#e0e4ea';
  const cardBg = dark ? '#1a1a1a' : '#fff';
  const navBg = dark ? 'rgba(17,17,17,0.9)' : 'rgba(253,244,255,0.85)';
  ...
  return (
    <div style={{ background: bg, color: text, minHeight: '100vh', fontFamily: 'system-ui, sans-serif', transition: 'background 0.2s, color 0.2s' }}>
      ...
      <nav className="nav-inner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 32px', borderBottom: `0.5px solid ${border}`, background: navBg, backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        {/* Mobile Hamburger */}
        <div className="mobile-ham" style={{ alignItems: 'center', gap: 10 }}>
          <button onClick={toggleTheme} style={{ width: 34, height: 34, borderRadius: 8, border: `0.5px solid ${btnOutlineBorder}`, background: btnOutlineBg, cursor: 'pointer', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {dark ? '☀️' : '🌙'}
          </button>
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ width: 36, height: 36, borderRadius: 8, border: `0.5px solid ${border}`, background: btnOutlineBg, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
            {menuOpen ? '✕' : '☰'}
          </button>
        </div>
      </nav>
  ...
  <a href="#" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 20px', fontSize: 14, color: text, textDecoration: 'none' }}>
    📝 Blog
      ...
      <div className="hero-pad" style={{ textAlign: 'center', padding: '72px 32px 56px' }}>
        <h1 className="hero-title" style={{ fontSize: 44, fontWeight: 500, lineHeight: 1.15, letterSpacing: '-1.5px', marginBottom: 18, maxWidth: 600, margin: '0 auto 18px' }}>
          Stop guessing. Start creating videos that{' '}
          <span style={{ color: '#1B4FDB' }}>actually blow up.</span>
        </h1>
        <p className="hero-sub" style={{ fontSize: 17, color: sub, maxWidth: 500, margin: '0 auto 36px', lineHeight: 1.7 }}>
      ...
      {/* Video */}
      <div className="section-pad" style={{ padding: '0 32px', marginBottom: 56 }}>
      ...
      {/* Stats */}
      <div className="stats-grid" style={{ gap: 1, background: dark ? '#2a2a2a' : '#e0e4ea', borderTop: `0.5px solid ${border}`, borderBottom: `0.5px solid ${border}`, marginBottom: 56 }}>
        {[{ num: '10,000+', label: 'Topics analyzed' }, { num: '500+', label: 'YouTube creators' }, { num: 'Hi & En', label: 'Language support' }].map((s, i) => (
          <div key={i} style={{ background: dark ? '#1a1a1a' : 'rgba(255,255,255,0.7)', padding: 20, textAlign: 'center' }}>
      ...
      {/* Features */}
      <div className="section-pad" style={{ padding: '0 32px', marginBottom: 64 }}>
        ...
        <div className="feat-grid">
          {features.map((f, i) => (
            <div key={i} style={{ background: dark ? '#1a1a1a' : 'rgba(255,255,255,0.8)', border: `0.5px solid ${border}`, borderRadius: 12, padding: 16, position: 'relative' }}>
      ...
      <div className="section-pad" style={{ padding: '0 32px', marginBottom: 64 }}>
        ...
        <div className="price-grid">
          <div style={{ background: dark ? '#1a1a1a' : 'rgba(255,255,255,0.8)', border: `0.5px solid ${border}`, borderRadius: 12, padding: 24 }}>
      ...
      {/* CTA */}
      <div className="cta-margin" style={{ margin: '0 32px 32px', border: `0.5px solid ${border}`, borderRadius: 12, padding: '48px 32px', textAlign: 'center', background: dark ? '#1a1a1a' : 'rgba(255,255,255,0.6)' }}>
